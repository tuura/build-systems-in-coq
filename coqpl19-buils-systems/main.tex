...
\vspace{-1mm}
\begin{minted}{coq}
(* Total Tasks restrict the key of the dependencies to be strictly
...
task descriptions as the~\hs{TotalTasks} datatype. In contrasts with the regular,
possibly non-terminating \hs{Tasks}, the total task descriptions are represented as
...
by establishing a well-founded order on~\hs{TotalTasks}:
...
Definition depth
  {C : (Type -> Type) -> Type}
  {F : (Type -> Type)} `{CF: C F}
  (key : nat) (tasks : TotalTasks C) : nat :=
...
an idea of proof of termination of the~\hs{busy} build system for total tasks:
if the~\hs{TotalTasks} contains~\hs{Nothing} then the key is an input, hence the
...
supposingly always terminating~\hs{TotalTasks} has not made the problem easier.
