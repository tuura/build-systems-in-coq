...
Record Task (C : (Type -> Type) -> Type) (K : Type) (V : Type) := {
  key : K;
  run : forall {F} `{CF: C F}, Maybe ((K -> F V) -> F V)}.
...
Definition Build (C : (Type -> Type) -> Type) (I K V : Type) :=
  Task C K V -> K -> Store I K V -> Store I K V.

(* A total Task restricts the key of the dependencies (if any) to be strictly
* smaller than the top-level one *)
Record TotalTask (C : (Type -> Type) -> Type) := {
  key : nat;
  run : forall {F} `{CF: C F}, Maybe ((Fin.t key -> F nat) -> F nat)}
...
Definition TotalBuild (C : (Type -> Type) -> Type) (I K V : Type) :=
  TotalTask C K V -> K -> Store I K V -> Store I K V.
\end{minted}
